...
// set up as well.
  ...
  function setUpTextures(){
      ...
      if( curTexture === "myimage" ) {
          huskyTexture = gl.createTexture();

          // get some texture space from the gpu
          gl.bindTexture( gl.TEXTURE_2D, huskyTexture);

          // load the actual image
          var huskyImage = document.getElementById('husky-texture')
          huskyImage.crossOrigin = "";

          // load the texture data
          gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, huskyImage.width, huskyImage.height, 0, gl.RGBA, gl.UNSIGNED_BYTE, huskyImage);
...
function bindVAO (shape, program) {
  ...
  function init() {
    ...
    sphereGlobeProgram = initProgram('sphereMap-V', 'sphereMap-F');
      huskyProgram = initProgram('sphereMap-V', 'huskyMap-F');
